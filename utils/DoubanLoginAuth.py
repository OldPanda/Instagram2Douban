...
class DoubanOAuth2Mixin(DoubanMixin):
    ...
    def _on_access_token(self, future, response):
        ...
        self.token = args # define a global variable token to save tokens msg
    ...
    def _on_get_user_request(self, future, response):
        if response.error:
            ...
            return
        # combine token dict with user dict
        self.token.update(escape.json_decode(response.body))
        future.set_result(self.token)
